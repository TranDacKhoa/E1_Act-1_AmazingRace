...
Font_Winner = pygame.font.SysFont('consolas', 50)
...
class Start():
    ...
    def draw(Start):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if event.button == 1 and Mouse_x >= 447 and Mouse_x <= 760 and Mouse_y >= 357 and Mouse_y <= 485:
                Start.start = False
...
class Map_Select():
    def __init__(Map_Select):
        ...
        Map_Select.Mouse_x = 0
        Map_Select.Mouse_y = 0

        Map_Select.Day_x1 = 228
        Map_Select.Day_x2 = 497
        Map_Select.Day_y1 = 426
        Map_Select.Day_y2 = 509

        Map_Select.Night_x1 = 707
        Map_Select.Night_x2 = 947
        Map_Select.Night_y1 = 426
        Map_Select.Night_y2 = 509
    ...
    def draw(Map_Select):
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            ...
            if event.button == 1 and Map_Select.Mouse_x >= Map_Select.Day_x1 and Map_Select.Mouse_x <= Map_Select.Day_x2 and Map_Select.Mouse_y >= Map_Select.Day_y1 and Map_Select.Mouse_y <= Map_Select.Day_y2:
                Character_Select.start = True
                Map_Select.start = False
                map.Day = True
            if event.button == 1 and Map_Select.Mouse_x >= Map_Select.Night_x1 and Map_Select.Mouse_x <= Map_Select.Night_x2 and Map_Select.Mouse_y >= Map_Select.Night_y1 and Map_Select.Mouse_y <= Map_Select.Night_y2:
                Character_Select.start = True
                Map_Select.start = False
                map.Night = True
...
class map():
    def __init__(map):
        ...
        map.Day = False
        map.Night = False
        map.surface1 = pygame.image.load('Sanhok-night.png')
        map.surface2 = pygame.image.load('Sannhok.png')
...
class Winner():
    ...
    def draw(Winner):
        ...
        if Dinosaurs.x + 50 >= WINDOWWIDTH and (Winner.dem == 0 or Winner.dem == 3):
            ...
            Winner.dem = 3
        if Ninja.x >= WINDOWWIDTH and (Winner.dem == 0 or Winner.dem == 4):
            window.blit(Winner.surface4, (Winner.x, Winner.y))
            Winner.dem = 4
        if Gold_Dog.x >= WINDOWWIDTH and (Winner.dem == 0 or Winner.dem == 5):
            window.blit(Winner.surface5, (Winner.x, Winner.y))
            Winner.dem = 5
        if Grey_Cat.x >= WINDOWWIDTH and (Winner.dem == 0 or Winner.dem == 6):
