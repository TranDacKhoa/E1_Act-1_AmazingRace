            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_RETURN:
                    if Login.Check_Login(Login.text_user):
                        ...
                        Login.Update_value('A' + str(Login.so_tk + 1), str(Login.text_user))
                        Login.so_tk += 1
...
class Pause():
    def __init__(Pause):
        ...
        Pause.back = pygame.Rect(554, 223, 100, 160)
        Pause.home = pygame.Rect(554, 383, 100, 160)
...
class Winner():
    ...
    def draw(Winner):
        if Winner.i <= 5:
            if Character_Select.start1:
                if Cowboy.x + 165 >= WINDOWWIDTH and Winner.dem[0] == 0:
                    Winner.xep_hang_so[Winner.i] = 1
                    Winner.i += 1
                    Winner.dem[0] = 1
                if Cowgirl.x + 165 >= WINDOWWIDTH and Winner.dem[1] == 0:
                    Winner.xep_hang_so[Winner.i] = 2
                    Winner.i += 1
                    Winner.dem[1] = 1
                if Dinosaurs.x + 165 >= WINDOWWIDTH and Winner.dem[2] == 0:
                    Winner.xep_hang_so[Winner.i] = 3
                    Winner.i += 1
                    Winner.dem[2] = 1
                if Ninja.x + 165 >= WINDOWWIDTH and Winner.dem[3] == 0:
                    Winner.xep_hang_so[Winner.i] = 4
                    Winner.i += 1
                    Winner.dem[3] = 1
                if Gold_Dog.x + 165 >= WINDOWWIDTH and Winner.dem[4] == 0:
                    Winner.xep_hang_so[Winner.i] = 5
                    Winner.i += 1
                    Winner.dem[4] = 1
                if Grey_Cat.x + 165 >= WINDOWWIDTH and Winner.dem[5] == 0:
                    Winner.xep_hang_so[Winner.i] = 6
                    Winner.i += 1
                    Winner.dem[5] = 1

            if Character_Select.start2:
                if Dac_Khoa.x + 165 >= WINDOWWIDTH and Winner.dem[0] == 0:
                    Winner.xep_hang_so[Winner.i] = 1
                    Winner.i += 1
                    Winner.dem[0] = 1
                if Quang_Khoa.x + 165 >= WINDOWWIDTH and Winner.dem[1] == 0:
                    Winner.xep_hang_so[Winner.i] = 2
                    Winner.i += 1
                    Winner.dem[1] = 1
                if An_Nguyen.x + 165 >= WINDOWWIDTH and Winner.dem[2] == 0:
                    Winner.xep_hang_so[Winner.i] = 3
                    Winner.i += 1
                    Winner.dem[2] = 1
                if Gia_Long.x + 165 >= WINDOWWIDTH and Winner.dem[3] == 0:
                    Winner.xep_hang_so[Winner.i] = 4
                    Winner.i += 1
                    Winner.dem[3] = 1
                if Khoi_Nguyen.x + 165 >= WINDOWWIDTH and Winner.dem[4] == 0:
                    Winner.xep_hang_so[Winner.i] = 5
                    Winner.i += 1
                    Winner.dem[4] = 1
                if Dinh_Loc.x + 165 >= WINDOWWIDTH and Winner.dem[5] == 0:
                    Winner.xep_hang_so[Winner.i] = 6
                    Winner.i += 1
                    Winner.dem[5] = 1
            ...
            if Character_Select.start1:
                for i in range(0, 5):
                    ...
                    if i == 0:
                        ...
                        window.blit(Winner.surface_End_chu, (500, 210))
                    else:
                        Winner.surface_End_chu = Font_sans30.render(Winner.xep_hang_chu[i], True, BLACK)
                        window.blit(Winner.surface_End_chu, (500, 250 + 53 * i))
            else:
                for i in range(0, 5):
                    Winner.xep_hang_chu[i] = CharacterName2[Winner.xep_hang_so[i]]
                if i == 0:
                    Winner.surface_End_chu = Font_sans30.render(Winner.xep_hang_chu[i], True, BLACK)
                    window.blit(Winner.surface_End_chu, (500, 210))
                else:
